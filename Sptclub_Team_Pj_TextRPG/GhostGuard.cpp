#include "GhostGuard.h"
#include "Player.h"
#include "GameUI.h"

#include <cstdlib>


GhostGuard::GhostGuard()
	: Monster("원혼 시위군", 6, 60, "낡은 호위 무사의 검", 50, 15,
		"왕을 지키지 못한 시위군의 원혼이 차가운 칼날과 함께 나타났다!"){}

void GhostGuard::Attack(Player* player)
{
	PrintStory(0, "원혼 시위군은 억울한듯 차가운 검을 휘둘렀다!");

	int damage = CalculateDamage();

	player->TakeDamage(damage);
	PrintStory(1, std::to_string(damage) + "의 피해를 입었다!");
	PrintStory(2, "남은 HP : " + std::to_string(player->getHp()));

	WaitForEnter();
}


void GhostGuard::Skill(Player* player)
{
	const int skillCost = 50;

	if (getMp() < skillCost)
	{
		Attack(player);
		return;
	}

	setMp(getMp() - skillCost);
	PrintStory(1, "원혼 시위군은 왕을 지키지 못한 원한으로 강력한 검격을 날렸다!");

	int damage = CalculateSkillDamage(1.5f);

	player->TakeDamage(damage);
	PrintStory(0, std::to_string(damage) + "의 피해를 입었다!");
	PrintStory(2, "남은 HP : " + std::to_string(player->getHp()));
	WaitForEnter();
}